vim.api.nvim_create_autocmd('LspAttach', {
  callback = function(args)
    ...
    -- diagnostics
    vim.keymap.set("n", "[d", function() vim.diagnostic.jump({ count = -1, wrap = false }) end,
    ...
    -- project diagnostics
    vim.keymap.set("n", "<leader>wa", function() fzfLua.diagnostics_workspace({ winopts = winoptsConfig }) end,
      { desc = "lsp: workspace diagnostics" })
    vim.keymap.set("n", "<leader>we",
      function() fzfLua.diagnostics_workspace({ severity_only = vim.diagnostic.severity.ERROR, winopts = winoptsConfig }) end,
      { desc = "lsp: workspace error diagnostics" })
    vim.keymap.set("n", "<leader>ww",
      function() fzfLua.diagnostics_workspace({ severity_only = vim.diagnostic.severity.WARN, winopts = winoptsConfig }) end,
      { desc = "lsp: workspace warning diagnostics" })
    -- buffer diagnostics
    vim.keymap.set("n", "<leader>ba", function() fzfLua.diagnostics_document({ winopts = winoptsConfig }) end,
      { desc = "lsp: buffer diagnostics" })
    vim.keymap.set("n", "<leader>be",
      function() fzfLua.diagnostics_document({ severity_only = vim.diagnostic.severity.ERROR, winopts = winoptsConfig }) end,
      { desc = "lsp: buffer error diagnostics" })
    vim.keymap.set("n", "<leader>bw",
      function() fzfLua.diagnostics_document({ severity_only = vim.diagnostic.severity.WARN, winopts = winoptsConfig }) end,
    ...
    vim.keymap.set("n", "<leader>in", function() vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled()) end,
